    ...
    <TouchableOpacity className="bg-gray-50 rounded-2xl p-5 mb-3 border border-gray-200 active:bg-gray-100">
      <View className="flex-row justify-between items-start mb-3">
        <View className="flex-1">
          <Text className="text-black font-semibold text-base mb-2">{appointment.service}</Text>
          <Text className="text-gray-600 text-sm">with {appointment.professional}</Text>
        </View>
        <View className="bg-green-50 rounded-full px-3 py-1">
          <Text className="text-green-600 text-xs font-medium">{appointment.status}</Text>
        </View>
      </View>
      <View className="flex-row items-center justify-between pt-3 border-t border-gray-200">
        <View className="flex-row items-center">
          <Calendar size={16} color="#666" />
          <Text className="text-gray-600 text-sm font-medium ml-2">{appointment.date}</Text>
        </View>
        <View className="flex-row items-center bg-white rounded-full px-3 py-1 border border-gray-200">
          <Clock size={14} color="#666" />
          <Text className="text-gray-600 text-sm font-medium ml-2">{appointment.time}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="px-6 pt-12 pb-6 bg-white">
          ...
          <View className="items-center mb-8">
            ...
            <View className="flex-row items-center">
              ...
              <Text className="text-gray-600 text-sm ml-2">{userData.location}</Text>
            </View>
          </View>
        ...
        <View className="px-6 mb-8">
          <View className="flex-row items-center justify-between mb-5">
            <Text className="text-xl font-light text-black">Upcoming Appointments</Text>
            <TouchableOpacity>
        ...
        {/* Enhanced Menu Items */}
        <View className="px-6 mb-8">
          <Text className="text-xl font-light text-black mb-5">Settings & Preferences</Text>
          <View className="bg-gray-50 rounded-3xl p-2 border border-gray-200">
            {menuItems.map((item, index) => (
              <View key={index}>
                <MenuItem {...item} />
                {index < menuItems.length - 1 && <View className="border-b border-gray-200 mx-4" />}
              </View>
            ))}
          </View>
        </View>
        ...
        <TouchableOpacity className="flex-row items-center justify-center py-5 mx-6 mb-8 border border-gray-300 rounded-2xl active:bg-gray-50">
          ...
          <Text className="text-gray-600 text-base font-medium ml-3">Sign Out</Text>
        </TouchableOpacity>
